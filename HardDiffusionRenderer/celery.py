...
logger = logging.getLogger("HardDiffusionRenderer")
...
def schedule_health_check(worker):
    ...
    # from HardDiffusionRenderer.tasks import health_check
...
app = Celery("HardDiffusionRenderer")

# Using a string here means the worker doesn't have to serialize
# the configuration object to child processes.
# - namespace='CELERY' means all celery-related configuration keys
#   should have a `CELERY_` prefix.
app.config_from_object("HardDiffusionRenderer.celeryconfig", namespace="CELERY")
if "render_health" in sys.argv:
    # app.steps["worker"].add(WorkerHealthMonitor)
    raise RuntimeError("Health monitor disabled!")

# Load task modules from all registered Django apps.
import HardDiffusionRenderer.tasks  # noqa
